...
public class PagamentoService {
    ...
    public void inicializarMercadoPago() {
        ...
        if (!mercadoPagoConfigurado()) {
            logger.warning("Mercado Pago não configurado. Defina MERCADOPAGO_ACCESS_TOKEN para habilitar pagamentos.");
            return;
        }
        MercadoPagoConfig.setAccessToken(mercadoPagoAccessToken);
        logger.info("Mercado Pago SDK inicializado. Token carregado com sucesso.");
    }

    private void validarMercadoPagoConfigurado() {
        if (!mercadoPagoConfigurado()) {
            throw new IllegalStateException(
                    "Mercado Pago não configurado. Substitua SEU_TOKEN ou defina MERCADOPAGO_ACCESS_TOKEN."
            );
        }
    }
    ...
    private boolean mercadoPagoConfigurado() {
        return mercadoPagoAccessToken != null
                ...
                && !mercadoPagoAccessToken.toUpperCase().contains("SEU_TOKEN");
    }
