...
const VagasRH = ({
    ...
    const resumo = useMemo(() => {
        const ativas = vagasNormalizadas.filter(vaga => vaga.status === 'ativa').length;
        const pausadas = vagasNormalizadas.filter(vaga => vaga.status === 'pausada').length;
        const encerradas = vagasNormalizadas.filter(vaga => vaga.status === 'encerrada').length;
        const candidatos = vagasNormalizadas.reduce((acc, vaga) => acc + vaga.inscritos.length, 0);
        const entrevistas = vagasNormalizadas.reduce((acc, vaga) => acc + Math.ceil(vaga.inscritos.length / 3), 0);
    ...
    const atualizarVaga = (id, dados) => {
        setVagas(prev => prev.map(vaga =>
            vaga.id === id ? { ...vaga, ...dados } : vaga
        ));

        setVagaDetalhe(prev => (
            prev?.id === id ? normalizarVaga({ ...prev, ...dados }) : prev
        ));
    };
    ...
    const salvarEdicao = () => {
        ...
        atualizarVaga(vagaEditando.id, {
            cargo: form.cargo,
            skillsExigidas: form.skills.split(',').map(skill => skill.trim()).filter(Boolean),
            experienciaMinima: Number(form.experienciaMinima) || 1,
            modelo: form.modelo,
            tipo: form.tipo,
    ...
    return (
        <section className="vagas-rh-page">
            <div className="vagas-summary-grid">
                <div className="post vagas-summary-card">
                    <Briefcase size={20} />
                ...
                <div className="post vagas-summary-card">
                    ...
                    <strong>{resumo.candidatos}</strong>
                </div>
            ...
            <div className="post vagas-rh-filters">
                <div className="vagas-rh-filter-title">
                    <Filter size={16} />
                    <span>Filtros</span>
                </div>
            ...
            {vagasFiltradas.map(vaga => {
                ...
                const candidatos = candidatosDaVaga(vaga);
                ...
                return (
                    <div className="post vagas-rh-card" key={vaga.id}>
                        <div className="vagas-rh-card-header">
                            <div>
                                <div className="vagas-rh-title-row">
                                    <h4>{vaga.cargo}</h4>
                                    <span className={`vaga-status ${vaga.status}`}>
                                        {statusLabels[vaga.status]}
                                    </span>
                                    <span className={`prioridade prioridade-${vaga.prioridade}`}>
                                        {vaga.prioridade}
                                    </span>
                                </div>
                        ...
                        <div className="vagas-rh-metrics">
                            <div>
                                <p>Inscritos</p>
                                <strong>{vaga.inscritos.length}</strong>
                            </div>
                            ...
                            <div>
                                <p>Experiencia</p>
                                <strong>{vaga.experienciaMinima}+ anos</strong>
                            </div>
                        ...
                        <div className="vagas-rh-skills">
                            {vaga.skillsExigidas.length > 0 ? (
                                vaga.skillsExigidas.map((skill, index) => (
                        ...
                        <div className="post-actions rh-actions vagas-rh-main-actions">
                            <button onClick={() => setVagaDetalhe(vaga)}>
                                <FileText size={18} /> Detalhes
                            </button>

                            <button onClick={() => abrirCandidatosNoPainel(vaga.id)}>
                                <Users size={18} /> Candidatos
                            </button>

                            {vaga.status === 'ativa' && (
                                <button onClick={() => atualizarVaga(vaga.id, { status: 'pausada' })}>
                                    <PauseCircle size={18} /> Pausar
                                </button>
                            )}

                            {vaga.status === 'pausada' && (
                                <button onClick={() => atualizarVaga(vaga.id, { status: 'ativa' })}>
                                    <PlayCircle size={18} /> Reabrir
                                </button>
                            )}

                            {vaga.status !== 'encerrada' && (
                                <button onClick={() => atualizarVaga(vaga.id, { status: 'encerrada' })}>
                        ...
                        {candidatos.length > 0 && (
                            <div className="top-candidates">
                                {candidatos.slice(0, 3).map(candidato => {
                                    const analise = obterAnaliseIA(candidato, vaga);

                                    return (
                                        <div key={candidato.id}>
                                            <strong>{candidato.nome}</strong>
                                            <span style={{ color: analise.cor }}>
                                                {analise.score}% | {analise.recomendacao}
                                            </span>
                                        </div>
                                    );
                                })}
                ...
                <div className="modal-overlay">
                    <div className="vaga-modal">
                        <div className="vaga-modal-header">
                            ...
                            <button onClick={() => setVagaDetalhe(null)}>
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="vaga-modal-tabs">
                            {['resumo', 'candidatos', 'pipeline', 'configuracoes'].map(aba => (
                                <button
                                    key={aba}
                                    className={abaDetalhe === aba ? 'active' : ''}
                        ...
                        <div className="vaga-modal-body">
                            {abaDetalhe === 'resumo' && (
                                <>
                                    ...
                                    <h4>Requisitos</h4>
                                    <div className="vagas-rh-skills">
                                        {vagaDetalhe.skillsExigidas.map((skill, index) => (
                                            <span key={index}>{skill}</span>
                                        ))}
                                    </div>
                                </>
                            ...
                            {abaDetalhe === 'candidatos' && (
                                <div className="modal-candidates-list">
                                        ...
                                        return (
                                            <div key={candidato.id} className="modal-candidate-ia">
                                                ...
                                                {analise.pontosFortes?.length > 0 && (
                                                    <ul>
                                                        {analise.pontosFortes.slice(0, 2).map((ponto) => (
                                                ...
                                                {analise.perguntasEntrevista?.length > 0 && (
                                                    <div className="ia-questions">
                                                        ...
                                                        {analise.perguntasEntrevista.slice(0, 2).map((pergunta) => (
                                                            <span key={pergunta}>{pergunta}</span>
                                                        ))}
                                                    </div>
                                                )}
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => analisarCandidatoComGemini(candidato, vagaDetalhe)}
                                                    disabled={carregando}
                                                >
                                                    <Sparkles size={14} />
                                    ...
                                    {candidatosDaVaga(vagaDetalhe).length === 0 && (
                            ...
                            {abaDetalhe === 'configuracoes' && (
                                <div className="config-actions">
                                    <button onClick={() => abrirEdicao(vagaDetalhe)}>
                                        <Edit3 size={16} /> Editar vaga
                                    </button>
                                    <button onClick={() => duplicarVaga(vagaDetalhe)}>
                                        <Copy size={16} /> Duplicar vaga
                                    </button>
                                    <button className="danger" onClick={() => excluirVaga(vagaDetalhe.id)}>
                                        <AlertCircle size={16} /> Excluir vaga
                                    </button>
                                </div>
                            )}
                        </div>
            ...
            {vagaEditando && (
                <div className="modal-overlay">
                    <div className="vaga-modal">
                        <div className="vaga-modal-header">
                            <h3>Editar Vaga</h3>
                            <button onClick={() => setVagaEditando(null)}>
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="vaga-edit-grid">
                            ...
                            <input value={form.skills} onChange={e => setForm({ ...form, skills: e.target.value })} placeholder="Skills separadas por virgula" />
                            <input type="number" value={form.experienciaMinima} onChange={e => setForm({ ...form, experienciaMinima: e.target.value })} placeholder="Experiencia" />
                            ...
                            <textarea value={form.descricao} onChange={e => setForm({ ...form, descricao: e.target.value })} placeholder="Descricao da vaga" />
                        </div>
